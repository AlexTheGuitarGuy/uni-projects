...
int main()
{
    FILE *rezultat = fopen("D:\\numbers.txt", "w+");
    FILE *avg = fopen("average.txt", "w+");

    printf("\nintroduceti numere intregi in seturi de cate 5 sau o valoare diferita de integer pentru a continua:\n");
    int tmp, numere = 0, suma = 0;
    for (int i = 1; 1; i++)
    {
        if (scanf("%d", &tmp))
        {
            fprintf(rezultat, "%d ", tmp);
            suma += tmp;
            numere++;
            if (i % 5 == 0)
                fprintf(rezultat, "\n");
    ...
    fseek(rezultat, 0, SEEK_SET);
    for (int i = 1; i <= numere; i++)
    {
        if (fscanf(rezultat, "%d", &tmp))
        {
            fprintf(avg, "%d ", tmp);
            if (i % 5 == 0)
            {
