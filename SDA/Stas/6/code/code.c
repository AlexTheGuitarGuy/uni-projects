...
Casa introduce_struct()
{
    Casa in;
    printf("adresa: ");
    scanf("%s", in.adresa);
    printf("telefonul: ");
    scanf("%s", in.telefonul);
    printf("suprafata: ");
    scanf("%f", &in.suprafata);
    printf("nr_odai: ");
    scanf("%d", &in.nr_odai);
    printf("costul: ");
    scanf("%f", &in.costul);
...
bool insertStruct(Node **rootptr, Casa value)
{
    Node *root = *rootptr;
    ...
    if (strcmp(value.adresa, root->value.adresa) == 0)
        return false;
...
void show_struct(Casa in, int level)
{
    printTabs(level);
    printf("adresa: ");
    printf("%s\n", in.adresa);
...
Node *findStructRec(Node *root, char *value)
{
    if (root == NULL)
        return NULL;
    if (strcmp(root->value.adresa, value) == 0)
        return root;

    if (strcmp(value, root->value.adresa) < 0)
